...
editar.addEventListener('click', () => {
    ...
    if (verificarDatos(titulo.value, descripcion.value, cuatrimestre.value, carrera.value, grupo.value)) {
        alert('Llene el formulario para continuar con la modificación.');
    } else {
        ...
        peticion.open("POST", '../controladores/editarClase.php', true);
        peticion.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        peticion.send(`codigo=${codigo.value}&titulo=${titulo.value}&descripcion=${descripcion.value}&cuatrimestre=${cuatrimestre.value}&carrera=${carrera.value}&grupo=${grupo.value}`);
    }
});